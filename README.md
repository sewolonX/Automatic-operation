...
系统设置页（第 4 页）内置了一个隐藏的「恢复默认设置」按钮，用于一键清空本脚本在该域名下的所有存储数据。

#### 20.1 显示方式

该按钮**默认隐藏**，需要**在系统设置页（第 4 页）连续点击第 4 页图标 4 次**才会出现（2 秒内完成，超时计数归零）。

> 设计意图：作为破坏性操作，通过隐藏 + 多次点击的方式防止误触。

#### 20.2 操作流程

1. 点击按钮 → 按钮变为**红色脉冲**「再次点击确认恢复默认设置」
2. 5 秒内再次点击 → 执行重置，否则恢复初始状态
3. 切换到其他页面 → 按钮自动隐藏，确认状态重置

#### 20.3 重置内容

确认重置后，脚本会清除当前域名下所有以 `AUTO_OP_` 开头的 `localStorage` 键，包括：

| 存储键 | 内容 |
| --- | --- |
| `AUTO_OP_SHARED_<host>` | 全局共享状态（折叠/展开、页面位置、主题等） |
| `AUTO_OP_CFG_<host>_<0~9>` | 10 套独立配置（目标元素、参数、匹配规则） |
| `AUTO_OP_REFRESH_STATE_<host>` | 跨刷新临时状态 |

清除后自动执行 `location.reload()` 刷新页面，所有设置恢复默认值。

#### 20.4 按钮样式

- 宽度与输入框一致（`width: 100%`，`box-sizing: border-box`）
- 红色边框 + 红色文字（`--panel-missing-border`），hover 时红底白字
- 确认状态：红底白字 + 脉冲动画（`auto-op-pulse`）
...
```text
Automatic-operation/
├── Automatic-operation.js    # 主脚本（~5258 行），全部功能
