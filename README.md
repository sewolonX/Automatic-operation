...
7. **失败回滚**：应用失败时自动恢复备份配置（含高亮清除、UI 同步、`discoveredElements` 恢复）
...
| 页码 | data-page | 图标 | 内容 |
| --- | --- | --- | --- |
| 第 1 页 | 0 | 📄 操作 / 配置加载 | 多选模式、操作策略、目标列表、选取/开始按钮；再次点击页签切换为「配置加载」模式（导入/导出配置） |
...
5. **清理阶段**：移除旧目标的所有高亮 class，清空 `c.targets` 和 `c.discoveredElements`
